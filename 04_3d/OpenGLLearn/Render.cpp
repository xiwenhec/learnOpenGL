...
void Render::init() {
	...
	GLubyte *data = stbi_load("E:/study/openGL02/03_matrix/OpenGLLearn/res/girl.jpg", &width, &height, &nrChannels, 0);
	if (data) {
	...
	data = stbi_load("E:/study/openGL02/03_matrix/OpenGLLearn/res/floor.jpg", &width, &height, &nrChannels, 0);
	if (data) {
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0,GL_RGB,GL_UNSIGNED_BYTE,data);
		glGenerateMipmap(GL_TEXTURE_2D);
	}
	...
	int modelMatrixPos = glGetUniformLocation(glProgram, "model");
...
void Render::draw() {
	...
	glUseProgram(glProgram);
	glUniformMatrix4fv(transFormMatrixLocation, 1, GL_FALSE, glm::value_ptr(transMatrix));
	glBindVertexArray(VAO);

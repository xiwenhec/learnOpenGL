...
void Render::init() {
	...
	glm::mat4 projection;
	projection = glm::perspective(glm::radians(60.0f), 800.0f / 600.0f, 3.0f, 100.0f);

	modelMatrixLoc = glGetUniformLocation(glProgram, "model");
	glUniformMatrix4fv(modelMatrixLoc,1,GL_FALSE,glm::value_ptr(model));

	viewMatrixLoc = glGetUniformLocation(glProgram, "view");
	//glUniformMatrix4fv(viewMatrixLoc, 1, GL_FALSE, glm::value_ptr(view));

	projectMatrixLoc = glGetUniformLocation(glProgram, "projection");
...
void Render::draw() {
	...
	GLfloat camX = glm::cos(glfwGetTime()) * radius;
	GLfloat camZ = glm::sin(glfwGetTime()) * radius;
